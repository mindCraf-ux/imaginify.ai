...
const onSubmitHandler = async (e) =>{
  ...
  try {
    ...
    if(state === 'Login'){
      //in this Data we will get the response from the backend


      const {data} =await axios.post(backendUrl + '/api/user/login',  
        { email, password })
        //here we check the response from the backend using Data
      if(data.success){
        setToken(data.token)
        setUser(data.user)
        //we will store the token in browser local storage
...
return (
  <div className='fixed top-0 left-0 right-0 bottom-0 z-0 backdrop-blur-sm
